...
:start
echo [ %date% %time% ] >> %cd%\pwdget\log.txt
echo [ %date% %time% ]:  프로그램 실행완료 (실행 버전:%version%) >> %cd%\pwdget\log.txt
...
echo [ %date% %time% ]: %a% (%USERNAME%) >> %cd%\pwdget\pwd.txt
goto exit
...
:setupyes
cls
echo [ %date% %time% ]: * 프로그램을 삭제하려면 프로그램 폴더 (배치파일)에서 "pwdget" 폴더를 삭제하시면 됩니다! >> %cd%\pwdget\info.txt
echo [ %date% %time% ]: * 악용으로 인한 책임은 모두 본인에게 있음을 알립니다. >> %cd%\pwdget\info.txt
echo [ %date% %time% ]: * 초기설정창은 %temp%\pwdgen이 있는경우 표시되지 않고 pwdgen모드가 실행됩니다. >> %cd%\pwdget\info.txt
goto exit

:exit
cls
echo [ %date% %time% ] 프로그램이 종료되었습니다. (버전: %version%)
exit
